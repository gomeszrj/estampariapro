...
export const orderService = {
  async uploadFile(file: File, path: string): Promise<string> {
            ...
            for (const item of order.items) {
                if (item.productId && item.quantity > 0) {
                    try {
                        ...
                        console.error(`Failed to deduct physical stock for product ${item.productId}`, e);
                    }
                }
            }
        }
    ...
    async update(id: string, updates: Partial<Order>) {
        ...
        // Return physical stock if order is cancelled
                ...
                for (const item of order.items) {
                    if (item.productId && item.quantity > 0) {
                        try {
                            ...
                            console.error(`Failed to return physical stock for product ${item.productId}`, e);
                        }
                    }
                }
            } catch (error) {
                console.error("Failed to return physical stock:", error);
            }
        }
