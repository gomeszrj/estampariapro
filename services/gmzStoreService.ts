...
export const getTenantId = async () => {
  ...
  if (urlStore) {
    const { data, error } = await supabase.rpc('get_tenant_by_store_name', { p_store_name: urlStore });
    if (!error && data) {
      return data;
    }
  }
...
export const gmzStoreService = {
  ...
  async uploadArtwork(file: File): Promise<string> {
    try {
      const ext = file.name.split('.').pop();
      const path = `artworks/${Date.now()}_${Math.random().toString(36).substring(7)}.${ext}`;
      const { error } = await supabase.storage.from('store_artworks').upload(path, file, { upsert: true });
      if (error) throw error;
      const { data } = supabase.storage.from('store_artworks').getPublicUrl(path);
      return data.publicUrl;
    } catch (err) {
