...
export const whatsappService = {
    ...
    connectPairingCode: async (phoneNumber: string): Promise<string | null> => {
        ...
        try {
            ...
            const response = await fetch(`${baseUrl}/instance/connect/${encodeURIComponent(instanceName)}?number=${cleanPhone}`, {
                headers: { 
                    'apikey': apiKey,
                    'Bypass-Tunnel-Reminder': 'true'
                }
    ...
    getChats: async (): Promise<any[]> => {
        ...
        try {
            // Evolution API v2 fetchChats endpoint
            const response = await fetch(`${baseUrl}/chat/findChats/${encodeURIComponent(instanceName)}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'apikey': apiKey },
                body: JSON.stringify({})
            });
