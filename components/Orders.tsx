...
const Orders: React.FC<OrdersProps> = ({ orders, setOrders, products, clients, setClients, botDraft, onDraftUsed, isMasterAdmin }) => {
  ...
  // Suppliers & Margins — multi-supplier por item
  const [suppliers, setSuppliers] = useState<any[]>([]);
  const [supplierId, setSupplierId] = useState<string>(''); // fornecedor padrão (atalho "aplicar a todos")
  const [supplierCostsMap, setSupplierCostsMap] = useState<Map<string, number>>(new Map());

  // Estado por item: fornecedor e custo unitário (arrays paralelos ao parsedItems)
  ...
  const loadSuppliers = async () => {
    try {
      ...
      setSuppliers(data);
      // Carrega mapa de custos para todos os fornecedores de uma vez
      if (data.length > 0) {
        const map = await supplierService.getSupplierProductsMap(data.map((s: any) => s.id));
        setSupplierCostsMap(map);
      }
    } catch (e) { console.error('Erro ao buscar fornecedores:', e); }
  };

  // Ao mudar fornecedor de um item: auto-preenche custo se cadastrado
  const handleItemSupplierChange = async (idx: number, newSupplierId: string) => {
    setItemSupplierIds(prev => {
      const next = [...prev];
      next[idx] = newSupplierId;
      return next;
    });
    // Auto-preenche custo do produto neste fornecedor
    const item = parsedItems[idx];
    const prod = productsByName.get((item?.product || '').trim().toLowerCase());
    if (prod && newSupplierId) {
      const costKey = `${newSupplierId}__${prod.id}`;
      const cost = supplierCostsMap.get(costKey);
      setItemUnitCosts(prev => {
        const next = [...prev];
        next[idx] = cost !== undefined ? cost : undefined;
        return next;
      });
    } else {
      setItemUnitCosts(prev => {
        const next = [...prev];
        // Fallback para o custo interno (costPrice) cadastrado no próprio produto
        next[idx] = prod?.costPrice ? prod.costPrice : undefined;
        return next;
      });
    }
  };

  // Atualiza custo unitário manual de um item
  const handleItemUnitCostChange = (idx: number, cost: number | undefined) => {
    setItemUnitCosts(prev => {
      const next = [...prev];
      next[idx] = cost;
      return next;
  ...
  // Botão "Aplicar fornecedor padrão a todos os itens"
  const applyDefaultSupplierToAll = () => {
    if (!supplierId) return;
    const newSupplierIds = parsedItems.map(() => supplierId);
    const newCosts = parsedItems.map((item) => {
      const prod = productsByName.get((item.product || '').trim().toLowerCase());
      if (prod) {
    ...
    setItemSupplierIds(newSupplierIds);
    setItemUnitCosts(newCosts);
  };
  ...
  const removeItem = (index: number) => {
    ...
    setItemSupplierIds(prev => prev.filter((_, i) => i !== index));
    setItemUnitCosts(prev => prev.filter((_, i) => i !== index));
  };
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        clientId: clientsByName.get((clientName || '').toLowerCase())?.id || null,
        ...
        // Custo real: soma de unit_cost × qty de cada item (multi-fornecedor)
        totalCost: parsedItems.reduce((acc, curr, idx) => {
  ...
  const tabCounts = {
    ...
    conferido: contextOrders.filter(o => o.status === OrderStatus.FINALIZATION).length,
  ...
  const getStatusLabel = (status: OrderStatus) => {
    if (status === OrderStatus.RECEIVED) return 'Pedido Recebido';
    if (status === OrderStatus.IN_PRODUCTION) return 'Em Produção';
    if (status === OrderStatus.FINALIZATION) return 'Conferido';
                ...
                <div className={`grid grid-cols-1 ${isMasterAdmin ? 'lg:grid-cols-2' : 'max-w-3xl mx-auto'} gap-8 animate-in slide-in-from-left-4 duration-300`}>
                  <div className="space-y-6">
                    ...
                    <div className={`bg-[#0b1221] p-6 rounded-3xl border border-[#1e293b] space-y-6 ${isPartialEditMode ? 'opacity-60 pointer-events-none' : ''}`}>
                      ...
                      {/* Supplier & Margin Section — Atalho "Aplicar a Todos" */}
                      <div className="mt-4 pt-4 border-t border-[#1e293b]">
                        <div className="flex items-center gap-2 mb-3">
                          <Truck className="w-4 h-4 text-purple-400" />
                          <div>
                            <label className="text-[9px] font-bold text-purple-400 uppercase tracking-widest block">Fornecedor para todos os itens</label>
                            <p className="text-[8px] text-slate-600 mt-0.5">Atalho rápido — defina individualmente na aba <span className="text-purple-400 font-bold">Itens</span> para múltiplos fornecedores</p>
                          </div>
                        </div>
                        <div className="flex gap-3 items-center">
                          <select
                            ...
                            className="flex-1 bg-[#0b1221] border border-slate-700 rounded-xl px-3 py-3 text-[10px] font-black text-slate-200 uppercase outline-none focus:border-purple-500"
                          >
                          ...
                          <button
                            type="button"
                            disabled={!supplierId}
                            onClick={applyDefaultSupplierToAll}
                            className="px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest bg-purple-600/20 border border-purple-500/30 text-purple-400 hover:bg-purple-600/40 disabled:opacity-30 disabled:cursor-not-allowed transition-all whitespace-nowrap"
                          >
                            Aplicar a todos
                          </button>
                        </div>
                        {/* Indicador de itens com fornecedores distintos */}
                        {itemSupplierIds.filter(Boolean).length > 0 && (
                          <div className="mt-2 flex items-center gap-2 bg-purple-500/5 border border-purple-500/10 rounded-lg px-3 py-2">
                            <span className="text-[9px] font-bold text-purple-400">
                              {new Set(itemSupplierIds.filter(Boolean)).size > 1
                                ? `✓ ${new Set(itemSupplierIds.filter(Boolean)).size} fornecedores diferentes nos itens`
                                : `✓ Fornecedor aplicado a ${itemSupplierIds.filter(Boolean).length} item(ns)`
                              }
                            </span>
                          </div>
                        )}
...
value={internalNotes}
              ...
              {activeTab === 'items' && (
                <OrderItemsForm
                  ...
                  suppliers={suppliers}
                  supplierCostsMap={supplierCostsMap}
                  onItemSupplierChange={handleItemSupplierChange}
                  onItemUnitCostChange={handleItemUnitCostChange}
                  itemSupplierIds={itemSupplierIds}
                  itemUnitCosts={itemUnitCosts}
                />
              )}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-6">
        <div className="flex items-center gap-6 overflow-x-auto border-b border-[#1e293b] w-full md:w-auto">
          {[
            ...
            { label: 'Recebido', count: tabCounts.aguardando, value: OrderStatus.RECEIVED },
            { label: 'Produção', count: tabCounts.producao, value: OrderStatus.IN_PRODUCTION },
            { label: 'Conferido', count: tabCounts.conferido, value: OrderStatus.FINALIZATION },
      ...
      {displayOrders.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-6">
          {displayOrders.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map(order => {
            ...
            // Usa totalCost salvo (custo real por fornecedor/item), com fallback para costPrice
            const costTotal = (order.totalCost && order.totalCost > 0)
              ? order.totalCost
              : (order.items || []).reduce((acc, item) => {
                  const cost = (item as any).unitCost;
                  return acc + (cost !== undefined && cost > 0
                    ? cost * item.quantity
                    : (productsByName.get(item.productName.toLowerCase())?.costPrice || 0) * item.quantity);
                }, 0);
            const profit = order.totalValue - costTotal;
            const isPaid = order.paymentStatus === PaymentStatus.FULL;

            // Fornecedores distintos neste pedido
            const distinctSupplierIds = Array.from(new Set(
              (order.items || []).map(i => (i as any).supplierId).filter(Boolean)
            ));
            const hasMultipleSuppliers = distinctSupplierIds.length > 1;
