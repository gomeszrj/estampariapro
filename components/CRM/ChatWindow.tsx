...
import { Send, Paperclip, MoreVertical, Smile, Sparkles, Loader2 } from 'lucide-react';
...
export const ChatWindow: React.FC<ChatWindowProps> = ({ chatId, clientName, whatsappId }) => {
    ...
    const handleAiAssist = async () => {
        setIsAiLoading(true);
        try {
            const prompt = `Atue como um vendedor de estamparia extremamente simpático e persuasivo. Escreva uma mensagem curta (1 parágrafo) para reengajar esse cliente chamado ${clientName}. Use 1 ou 2 emojis. Seja natural.`;
            const copy = await aiService.generateChatResponse(prompt, '');
            setNewMessage(copy);
            notify.success('Copy gerada com sucesso!');
        } catch (e) {
            notify.error('Erro ao gerar mensagem com IA.');
        } finally {
            ...
            <div className="p-4 border-t border-[#1e293b] bg-[#0f172a]">
                <div className="flex gap-2 items-end">
                    ...
                    <button
                        ...
                        className="p-3 rounded-xl bg-purple-600/10 text-purple-400 border border-purple-500/20 hover:bg-purple-600/20 disabled:opacity-50 transition-all group relative"
                        title="Gerar Copy Persuasiva com IA"
                    >
