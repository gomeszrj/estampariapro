...
import { Plus, Search, Edit2, Trash2, Truck, Phone, X, Save, DollarSign, TrendingUp } from 'lucide-react';
          ...
          <div className="bg-[#0f172a] rounded-xl shadow-2xl w-full max-w-md border border-[#1e293b] flex flex-col max-h-[90vh]">
            ...
            {editingSupplier ? (
              <div className="p-6 border-t border-[#1e293b] bg-[#0b1121]">
                <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2"><DollarSign className="w-4 h-4 text-emerald-500" /> Tabela de Custos (por Produto)</h3>
                ...
                <div className="flex gap-2 mb-2">
                  ...
                  <input
                    ...
                    className="w-28 bg-[#1e293b] border border-[#334155] rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-purple-500"
                  />
                ...
                {/* Referência: custo interno do produto selecionado */}
                {selectedProductId && (() => {
                  const prod = allProducts.find(p => p.id === selectedProductId);
                  return prod && (prod.costPrice || 0) > 0 ? (
                    <div className="flex items-center gap-2 bg-[#151B2B] border border-[#1e293b] rounded-lg px-3 py-2 mb-3 text-xs">
                      <TrendingUp className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                      <span className="text-slate-500">Custo interno de produção:</span>
                      <span className="font-black text-rose-400">R$ {(prod.costPrice || 0).toFixed(2)}</span>
                      {prod.basePrice > 0 && (
                        <>
                          <span className="text-slate-600">|</span>
                          <span className="text-slate-500">Preço de venda:</span>
                          <span className="font-black text-emerald-400">R$ {prod.basePrice.toFixed(2)}</span>
                        </>
                      )}
                    </div>
                  ) : null;
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto pr-2">
                  {supplierProducts.length === 0 ? (
                    ...
                    supplierProducts.map(sp => {
                      const prod = allProducts.find(p => p.id === sp.product_id);
                      const basePrice = prod?.basePrice || 0;
                      const margin = basePrice > 0 ? (((basePrice - Number(sp.cost_price)) / basePrice) * 100) : null;
                      return (
                        <div key={sp.id} className="flex justify-between items-center bg-[#1e293b] border border-[#334155] p-2 rounded-lg">
                          <div className="min-w-0">
                            <span className="text-sm text-slate-300 block truncate">{sp.products?.name}</span>
                            {margin !== null && (
                              <span className={`text-[9px] font-bold ${
                                margin >= 30 ? 'text-emerald-400' : margin >= 10 ? 'text-amber-400' : 'text-rose-400'
                              }`}>
