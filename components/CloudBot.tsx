...
export const CloudBot: React.FC<CloudBotProps> = ({ onCreateOrder }) => {
    ...
    return (
        <div className="flex h-[calc(100vh-100px)] gap-6 animate-in fade-in duration-500">
            {/* LEFT: CHAT INTERFACE */}
            <div className="flex-1 bg-[#0f172a] rounded-2xl border border-[#1e293b] shadow-2xl flex flex-col overflow-hidden relative">
                <div className="p-4 bg-[#0f172a] border-b border-[#1e293b] flex justify-between items-center">
                    ...
                    {messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] p-4 rounded-2xl text-sm font-medium leading-relaxed ${msg.role === 'user'
                                ? 'bg-white/10 text-white rounded-br-none shadow-lg shadow-black/20'
                ...
                <div className="p-4 bg-[#0f172a] border-t border-[#1e293b]">
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-[#0b1221] border border-[#1e293b] rounded-xl pl-4 pr-12 py-4 text-slate-200 focus:outline-none focus:border-slate-600 transition-colors font-medium"
                        />
                        <button
                            onClick={handleSend}
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white hover:bg-white/80 rounded-lg text-slate-950 transition-colors"
                        >
            ...
            <div className="w-[400px] bg-[#0f172a] rounded-2xl border border-[#1e293b] shadow-2xl flex flex-col p-6 space-y-6">
                ...
                <div className="bg-[#1C1C26] border border-[#1e293b] p-4 rounded-xl flex items-center gap-4">
                ...
                <div className="space-y-4">
                    <div className="bg-[#1C1C26] border border-[#1e293b] p-4 rounded-xl group hover:border-purple-500/50 transition-colors">
                    ...
                    <div className="bg-[#1C1C26] border border-[#1e293b] p-4 rounded-xl group hover:border-purple-500/50 transition-colors flex-1 min-h-[200px]">
                        ...
                        {context.draftOrderItems && context.draftOrderItems.length > 0 ? (
                            <ul className="space-y-2">
                                {context.draftOrderItems.map((item: any, idx: number) => (
                                    <li key={idx} className="text-xs text-slate-300 bg-[#0f172a] p-2 rounded border border-[#1e293b] flex justify-between">
