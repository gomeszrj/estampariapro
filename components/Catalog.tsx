...
const Catalog: React.FC<CatalogProps> = ({ products, setProducts, readOnly }) => {
  ...
  return (
    <>
      <div className="space-y-6 animate-in zoom-in-95 duration-500 relative z-0">
        {!readOnly ? (
                ...
                <div className="flex-1 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {/* Visual representation of Grades with Dimensions */}
                      {Object.entries(publicViewingProduct.allowedGrades || {}).map(([gradeLabel, sizes]: any) => (
                        <div key={gradeLabel} className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                          <span className="text-[9px] text-slate-400 block mb-2 uppercase tracking-wider font-bold">{gradeLabel}</span>
                          <div className="space-y-1.5">
                            {sizes.map((s: string) => {
                              const stats = publicViewingProduct.measurements?.[s] || {};
                              return (
                                <div key={s} className="flex items-center justify-between text-[9px] bg-slate-950 px-2 py-1 rounded-lg">
                                  <span className="font-black text-slate-200">{s}</span>
                                  <span className="text-slate-500">
                                    {stats.height ? `${stats.height}A x ${stats.width}L` : '---'}
                                  </span>
                                </div>
                              );
                            })}
