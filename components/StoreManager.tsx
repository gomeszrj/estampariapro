...
export const StoreManager: React.FC = () => {
  ...
  const generateAdminWhatsappMessage = (o: GmzOrder, stLabel: string) => {
    ...
    if (o.notes) msg += `📍 *Endereço/Obs:* ${o.notes.replace('Endereço: ', '')}\n`;
    
    msg += `\n*RESUMO DO PEDIDO:*\n`;
    o.items.forEach((item: any) => {
      msg += `• ${item.title || item.product?.title || 'Item'}\n`;
      if (item.personalizationType && item.personalizationType !== 'none' && item.details && item.details.length > 0) {
        item.details.forEach((d: any) => {
          msg += `  - ${d.name || 'Sem nome'}`;
          if (item.personalizationType === 'name_number') msg += ` (Nº ${d.number})`;
          msg += ` - Tam: ${item.size || 'Único'}\n`;
        });
      } else {
        msg += `  - Qtd: ${item.qty || 1}x | Tam: ${item.size || 'Único'}\n`;
      }
    });

    msg += `\n💰 *Total Estimado:* R$ ${Number(o.total_price).toFixed(2).replace('.', ',')}\n`;
    
    if (!o.custom_names) {
      msg += `\n⚠️ *ATENÇÃO:* Notei que você ainda não enviou o arquivo da sua arte. Por favor, envie o arquivo aqui pelo WhatsApp para que possamos dar andamento na produção do seu pedido!\n`;
    }
          ...
          {filteredOrders.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {filteredOrders.map(o => {
                ...
                return (
                  <div key={o.id} style={{ background: 'rgba(13,15,23,0.8)', border: isExpanded ? '1px solid rgba(124,58,237,0.4)' : '1px solid rgba(255,255,255,0.06)', borderRadius: 18, overflow: 'hidden', transition: 'all 0.2s' }}>
                    {/* Header summary (always visible) */}
                    <div 
                      onClick={() => setExpandedOrder(isExpanded ? null : o.id)}
                      style={{ padding: '18px 20px', cursor: 'pointer', display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}
                    >
                      {/* Status icon */}
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
                        <p style={{ fontSize: 20, fontWeight: 900, color: 'white' }}>R$ {Number(o.total_price).toFixed(2).replace('.', ',')}</p>
                    ...
                    {isExpanded && (
                      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '20px', background: 'rgba(0,0,0,0.2)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
                          ...
                          <div>
                            <h4 style={{ fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', marginBottom: 12, letterSpacing: '0.05em' }}>Dados do Pedido</h4>
                            
                            {o.notes && (
                              <div style={{ marginBottom: 12, background: 'rgba(255,255,255,0.03)', padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.05)' }}>
                                <p style={{ fontSize: 12, color: '#94a3b8', fontWeight: 700, marginBottom: 4 }}>Observações do Cliente:</p>
                                <p style={{ fontSize: 13, color: 'white' }}>{o.notes}</p>
                              </div>
                            )}

                            {o.custom_names && (
                              <div style={{ marginBottom: 16, background: 'rgba(124,58,237,0.1)', padding: '12px 14px', borderRadius: 10, border: '1px solid rgba(124,58,237,0.2)' }}>
                                <p style={{ fontSize: 12, color: '#a78bfa', fontWeight: 700, marginBottom: 6 }}>Arte Enviada (Upload):</p>
                                <a href={o.custom_names} target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: 'white', textDecoration: 'underline', display: 'flex', alignItems: 'center', gap: 6, wordBreak: 'break-all' }}>
                                  <ExternalLink size={14} /> {o.custom_names.split('/').pop() || 'Acessar Arquivo da Arte'}
                                </a>
                              </div>
                            )}

                            <div style={{ marginBottom: 16 }}>
                              <p style={{ fontSize: 12, color: '#94a3b8', fontWeight: 700, marginBottom: 6 }}>Atualizar Status:</p>
                              <select
                                value={o.status}
                                onChange={e => updateOrderStatus(o.id, e.target.value as any)}
                                style={{ ...inputStyle, fontSize: 13, padding: '10px 14px', color: st.color, background: st.bg, border: `1px solid ${st.color}40`, fontWeight: 700 }}
                              >
                            ...
                            {o.customer_phone && (
                              <a href={`https://wa.me/55${o.customer_phone.replace(/\D/g, '')}?text=${generateAdminWhatsappMessage(o, st.label)}`} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 14, background: '#25d366', color: 'white', padding: '12px', borderRadius: 12, textDecoration: 'none', fontWeight: 800, transition: 'all 0.2s', boxShadow: '0 4px 15px rgba(37,211,102,0.3)' }}>
                                💬 CHAMAR NO WHATSAPP E CONFIRMAR
                              </a>
                            )}
                          </div>
                          ...
                          {/* Coluna 2: Itens */}
                          <div>
                            <h4 style={{ fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', marginBottom: 12, letterSpacing: '0.05em' }}>Itens Solicitados ({o.items.length})</h4>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                              {o.items.map((item: any, i: number) => (
                                <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 12, padding: '12px' }}>
                                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                    <p style={{ fontSize: 13, fontWeight: 800, color: 'white' }}>{item.title || item.product?.title || 'Item'}</p>
                                    <p style={{ fontSize: 13, fontWeight: 800, color: '#a78bfa' }}>R$ {(Number(item.price || item.product?.price || 0) * (item.qty || 1)).toFixed(2).replace('.', ',')}</p>
                                  </div>
                                  <div style={{ display: 'flex', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
                                    <span style={{ fontSize: 11, color: '#94a3b8', background: 'rgba(0,0,0,0.3)', padding: '2px 6px', borderRadius: 4 }}>Qtd: {item.qty || 1}</span>
                                    <span style={{ fontSize: 11, color: '#94a3b8', background: 'rgba(0,0,0,0.3)', padding: '2px 6px', borderRadius: 4 }}>Tam: {item.size || 'Único'}</span>
                                  </div>
                                  
                                  {item.personalizationType && item.personalizationType !== 'none' && item.details && item.details.length > 0 && (
                                    <div style={{ marginTop: 8, padding: '8px', background: 'rgba(0,0,0,0.2)', borderRadius: 8 }}>
                                      <p style={{ fontSize: 10, fontWeight: 800, color: '#64748b', textTransform: 'uppercase', marginBottom: 4 }}>Nomes/Personalização:</p>
                                      {item.details.map((d: any, idx: number) => (
                                        <p key={idx} style={{ fontSize: 11, color: '#e2e8f0', display: 'flex', gap: 6 }}>
                                          <span style={{ color: '#475569' }}>#{idx+1}</span>
                                          <span>{d.name || 'Sem nome'}</span>
                                          {item.personalizationType === 'name_number' && <span style={{ color: '#a78bfa', fontWeight: 700 }}> (Nº {d.number})</span>}
                                        </p>
                                      ))}
                                    </div>
                                  )}
      ...
      {activeTab === 'configuracoes' && (
        <div style={{ maxWidth: 680 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            {[
              {
                title: 'Informações Gerais e Preços', icon: Globe,
                fields: [
                  ...
                  { key: 'personalization_name_price', label: 'Acréscimo: Apenas Nome (R$)', type: 'number' },
                  { key: 'personalization_name_number_price', label: 'Acréscimo: Nome + Número (R$)', type: 'number' },
                ],
              },
              {
