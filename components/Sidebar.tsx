...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isOpen, setIsOpen, isMasterAdmin, tenantId, userProfile }) => {
  ...
  const loadSettings = async () => {
    try {
      ...
      if (tenantId) {
        // Obter nome público da loja para o link externo
        const publicSettings = await supabase.from('gmz_store_settings').select('store_name').eq('tenant_id', tenantId).maybeSingle();
        if (publicSettings.data?.store_name) {
          setCompanyName(publicSettings.data.store_name); // Overwrite company name for the store link
        }
      }
    } catch (e) {
      console.error(e);
  ...
  const menuItems = isMasterAdmin
    ...
    : [
      ...
      // can('can_view_crm') && { id: 'crm', label: 'Evolution CRM', icon: MessageSquare }, // OCULTO — não em uso no momento
        ...
        <nav className="flex-1 p-6 space-y-3 overflow-y-auto">
          ...
          {(isMasterAdmin || permsLoaded) && menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all duration-300 ${activeView === item.id
          ...
          {!isMasterAdmin && (isMasterAdmin || (permsLoaded && permissions)) && (
            <div className="pt-4 mt-4 border-t border-[#1e293b] space-y-2">
              ...
              <div className="w-full flex items-center justify-between px-5 py-3 rounded-2xl text-white/70 border border-transparent hover:border-[#1e293b] hover:bg-white/5 hover:text-white transition-all group">
                <a href={`/catalogo${companyName ? `?store=${encodeURIComponent(companyName)}` : ''}`} target="_blank" className="flex items-center gap-4 flex-1">
                ...
                <button
                  onClick={() => handleCopyLink(`/catalogo${companyName ? `?store=${encodeURIComponent(companyName)}` : ''}`)}
              ...
              <div className="w-full flex items-center justify-between px-5 py-3 rounded-2xl text-white/70 border border-transparent hover:border-[#1e293b] hover:bg-white/5 hover:text-white transition-all group">
                <a href={`/?view=client_portal${companyName ? `&store=${encodeURIComponent(companyName)}` : ''}`} target="_blank" className="flex items-center gap-4 flex-1">
                ...
                <button
                  onClick={() => handleCopyLink(`/?view=client_portal${companyName ? `&store=${encodeURIComponent(companyName)}` : ''}`)}
                  className="opacity-0 group-hover:opacity-100 p-2 hover:bg-white/10 rounded-lg transition-all"
                  title="Copiar Link"
                >
                  <Copy className="w-4 h-4 text-white/50 hover:text-white" />
